    ...
    if row_idx in merged_a:
        ...
        if date_val is not None:
            ...
            if isinstance(date_val, (int, float)):
                ...
                # Handle formats like "2024-06-03", "2024/06/03", "6月3日" etc.
                date_str = date_val.strip()
                # Try YYYY-MM-DD or YYYY/MM/DD
                if re.match(r'\d{4}[-/]\d{1,2}[-/]\d{1,2}', date_str):
                    current_date = date_str[:10].replace('/', '-')
                else:
                    # Try to parse other formats
                    try:
                        # Try "X月X日" format
                        m = re.search(r'(\d{1,2})月(\d{1,2})[日号]?', date_str)
                        if m:
                            # Need year - use previous valid date's year or default to 2024
                            year = 2024  # Default
...
for r in raw_records:
    ...
    if song not in audience_details[aud]:
        audience_details[aud][song] = {'count': 0, 'dates': []}
    audience_details[aud][song]['count'] += 1
    audience_details[aud][song]['dates'].append(r['date'])
