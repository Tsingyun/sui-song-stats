...
for merge_range in merged_ranges:
    ...
    if merge_range.min_col == 1:  # A列
        date_merges.append((merge_range.min_row, merge_range.max_row, merge_range.min_col, merge_range.max_col))

print(f"\n找到 {len(date_merges)} 个日期合并区域:")
for merge in date_merges[:5]:
    print(f"  - 行 {merge[0]} 到 {merge[1]}")
...
for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):  # 从第2行开始（跳过表头）
    ...
    if date_value is not None:
        ...
        if isinstance(date_value, (int, float)):
            ...
            # 已经是日期格式
            current_date = str(date_value)[:10] if date_value else None
